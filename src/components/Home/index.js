...
const renderTopRatingConstraints = {
  initial: 'INITIAL',
  success: 'SUCCESS',
  fail: 'FAIL',
  loading: 'LOADING',
...
class Home extends Component {
  state = {
    ...
    renderTopRatingStatus: renderTopRatingConstraints.initial,
    originalMoviesList: [],
    trendingMoviesList: [],
    topRatingMoviesList: [],
    randomMovie: [],
  ...
  getTopRatingMoviesData = async () => {
    this.setState({renderTopRatingStatus: renderTopRatingConstraints.loading})
    const jwtToken = Cookies.get('jwt_token')
    const options = {
      method: 'GET',
      headers: {Authorization: `Bearer ${jwtToken}`},
    }
    const topRatingDataApi = 'https://apis.ccbp.in/movies-app/top-rated-movies'
    const response = await fetch(topRatingDataApi, options)
    if (response.ok === true) {
      const data = await response.json()
      const fetchedTopRatingData = data.results.map(eachMovie => ({
        backdropPath: eachMovie.backdrop_path,
        id: eachMovie.id,
        overview: eachMovie.overview,
        posterPath: eachMovie.poster_path,
      ...
      this.setState({
        topRatingMoviesList: fetchedTopRatingData,
        renderTopRatingStatus: renderTopRatingConstraints.success,
      })
    } else {
      this.setState({renderTopRatingStatus: renderTopRatingConstraints.fail})
    }
  }

  retryTopRatingMoviesData = () => {
    this.getTopRatingMoviesData()
  }
  ...
  renderTopRatingSuccessView = () => {
    const {topRatingMoviesList} = this.state

    return (
      <>
        <div className="movies-list-page">
          <Slider className="slick" {...settings}>
            {topRatingMoviesList.map(eachMovie => (
              <HomeMovieItems eachMovie={eachMovie} key={eachMovie.id} />
            ))}
          </Slider>
        </div>
  ...
  renderTopRatingErrorView = () => (
    <div className="error-page-container">
      <div className="thumbnail-error-page">
        ...
        <button
          onClick={this.retryTopRatingMoviesData}
          className="thumbnail-try-again-btn"
          type="button"
        >
          Try Again
  ...
  renderTopRatingSwitchViews = () => {
    const {renderTopRatingStatus} = this.state
    switch (renderTopRatingStatus) {
      case renderTopRatingConstraints.loading:
        return this.renderThumbnailLoadingView()
      case renderTopRatingConstraints.success:
        return this.renderTopRatingSuccessView()
      case renderTopRatingConstraints.fail:
        return this.renderTopRatingErrorView()
      default:
        return null
    }
  }
  ...
  render() {
    return (
      <>
        ...
        {this.renderTopRatingSwitchViews()}
